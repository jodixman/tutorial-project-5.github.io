    ...
    <section className={`${styles.flexCenter} ${styles.paddingY} flex-col`}>
      ...
      <div className={`${styles.flexStart} md:flex-row flex-col mb-8 w-full`}>
        <div className="flex-1 flex flex-col justify-start mr-10">
          ...
          <p className={`${styles.paragraph}`}>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Quam omnis nobis doloribus?</p>
        </div>
          ...
          {footerLinks.map((footerLink) => (
            <div key={footerLink.key} className='flex flex-col ss:my-0 my-4 min-w-[150px]'>
              ...
              {footerLink.links.map((Link, index) => (
                <li key={Link.name} className={`font-poppins font-normal text-[16px] leading-[27px] text-dimWhite hover:text-secondary cursor-pointer ${index !== footerLink.links.length - 1 ?'mb-4' : 'mb-0'}`}>
                    {Link.name}
                  </li>
                ))}
              </ul>
